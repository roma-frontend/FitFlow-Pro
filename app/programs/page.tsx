// app/programs/yoga.tsx (исправленная версия)
"use client";

import { Card, CardContent } from "@/components/ui/card";
import { programsData } from "@/lib/programs-data";
import { Heart, Target, Clock, Users, Star } from "lucide-react";
...
export default function ProgramsPage() {
  ...
  const getProgramCategory = (program: any): string => {
    ...
    if (
      title.includes("силов") ||
      description.includes("силов") ||
      title.includes("тренажер")
    )
      return "Силовые";
    if (
      title.includes("кардио") ||
      description.includes("кардио") ||
      title.includes("аэроб")
    )
      return "Кардио";
    if (title.includes("функциональн") || description.includes("функциональн"))
      return "Функциональный";
    if (
      title.includes("групп") ||
      description.includes("групп") ||
      program.participants.includes("группа")
    )
      return "Групповые";
    if (
      title.includes("персональн") ||
      description.includes("персональн") ||
      program.participants.includes("1-1")
    )
      return "Персональные";
