    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="left" className="w-80 p-0">
        <SheetHeader className="sr-only">
          <SheetTitle>Настройки панели администратора</SheetTitle>
        </SheetHeader>
...
const DesktopTabs = ({
  ...
  return (
    <div className="hidden md:block">
      <TabsList
        className="w-full h-auto p-1 bg-gray-50/80 backdrop-blur-sm flex overflow-x-auto scrollbar-hide gap-1 xs:grid xs:grid-cols-4 sm:flex sm:flex-wrap sm:justify-center sm:gap-2 sm:grid-cols-none sm:grid-rows-none md:grid md:grid-cols-3 md:grid-rows-3md:gap-1 lg:grid lg:grid-cols-4 lg:grid-rows-2 lg:gap-1 xl:grid xl:grid-cols-7 xl:grid-rows-1 2xl:grid-cols-8"
      >
        {tabsConfig.map((tab, index) => {
          ...
          return (
            <TabsTrigger
              key={tab.id}
              value={tab.id}
              className="flex flex-col items-center gap-1.5 p-3 data-[state=active]:bg-white data-[state=active]:shadow-sm transition-all duration-200 rounded-md min-w-[80px] flex-shrink-0 xs:min-w-0 xs:flex-shrink xs:w-full sm:min-w-[100px] md:w-full md:min-w-0 md:flex-1-0 lg:w-full lg:min-w-0 hover:bg-gray-100/50 hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-blue-500/20"
            >
              <Icon
                className="h-4 w-4 xs:h-3.5 xs:w-3.5 md:h-4 md:w-4 xl:h-5 xl:w-5"
              />
              <span
                className="text-xs font-medium text-center leading-tight text-[10px] xs:text-xs md:text-xs xl:text-sm max-w-full truncate"
              >
                {tab.label}
              </span>
            </TabsTrigger>
